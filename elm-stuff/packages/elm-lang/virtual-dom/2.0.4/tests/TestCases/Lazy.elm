module TestCases.Lazy exposing (..)
...
testLazyIdenticalRecord =
    test "isn't called again with identical record" <|
        let
            ...
            wrappedRender =
                unsafeRecordCallCount renderRecord

            v1 =
                renderDom <| lazy wrappedRender record

            v2 =
                updateDom v1 <| lazy wrappedRender record

            v3 =
                updateDom v2 <| lazy wrappedRender record
        in
...
testLazyIdenticalPrimitive =
    test "isn't called again with identical primitive" <|
        let
            wrappedRender =
                unsafeRecordCallCount renderPrimitive

            v1 =
                renderDom <| lazy wrappedRender 5

            v2 =
                updateDom v1 <| lazy wrappedRender 5

            v3 =
                updateDom v2 <| lazy wrappedRender 5
        in
            assertEqual 1 <| unsafeQueryCallCount wrappedRender
...
testLazyRecordMutationOfIdenticalValue =
    test "isn't called again with record mutation of identical value" <|
        let
            ...
            v2 =
                updateDom v1 <| lazy wrappedRender { record | x = "a" }

            v3 =
                updateDom v2 <| lazy wrappedRender { record | x = "a", y = "b" }
        in
            assertEqual 1 <| unsafeQueryCallCount wrappedRender
...
testNotLazyDifferentRecord =
    test "is called again with an equivalent but different record" <|
        let
            ...
            v1 =
                renderDom <| lazy wrappedRender { x = "a", y = "b" }

            v2 =
                updateDom v1 <| lazy wrappedRender { x = "a", y = "b" }

            v3 =
                updateDom v2 <| lazy wrappedRender { x = "a", y = "b" }
        in
